...
def make_choice(root_node):
    ...
    # return node_list[-1]
    return sorted(root_node.child_nodes.values(), key=lambda i: i.visits)[-1]
...
def traverse_nodes(node, state, identity):
    ...
    while not node.untried_actions and node.child_nodes:
        leaf_node = sorted(node.child_nodes.values(), key=lambda n: n.wins/n.visits + explore_faction * sqrt(2*log(n.parent.visits)/n.visits))[-1]
